...
func countMemory(mcBack *MemCallback, printChan chan printStruct, sizeLimit int, wg *sync.WaitGroup) {
	...
	for dat := range mcBack.redisChan {
		...
		default:
			processCompoundType(mcBack, dat, printChan, expSecond, expNsecond, sizeLimit)
...
func processCompoundType(mcBack *MemCallback, dat redisTypeGen, printChan chan printStruct, expSecond int64, expNsecond int64, sizeLimit int) {
	...
	if mcBack.memCalMap[dat.db][keyHsh].ifEndRecived {
		...
		if mcBack.memCalMap[dat.db][keyHsh].currentElemCnt == mcBack.memCalMap[dat.db][keyHsh].elements {
			if mcBack.memCalMap[dat.db][keyHsh].size >= sizeLimit {
...
func printResultSorted(mcBack *MemCallback, fd *bufio.Writer, printChan chan printStruct, sizeThreshold int, sortLimit int, wg *sync.WaitGroup, resultSep int) {
	//fmt.Println("printResult thread starts")
